            ...
            >
              {PROBLEM_STATEMENTS.map((prob) => (
                <motion.div 
                  key={prob.id}
                  variants={itemVariants}
                  className="bg-surface border border-destructive/20 rounded-2xl py-2.5 px-3 shadow-[0_4px_24px_rgba(217,74,86,0.05)] hover:border-destructive/40 transition-colors flex flex-row items-center gap-4"
                >
                  ...
                  <p className="text-[13px] text-text-muted/90 font-normal leading-relaxed m-0">
                    {prob.title}
                  </p>
                </motion.div>
              ))}
